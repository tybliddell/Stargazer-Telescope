...
class SSD1306:
    ...
    def set_text_and_show(self, string, core):
        self.lock.acquire()
        self.framebuf.fill_rect(0, int(core * self.height / 2), self.width, int(self.height / 2), 0)
        self.framebuf.text(f'C{core} {string}', 0, int(core * self.height / 2), 1)
        if len(string) > self.width // 8 - 3:
            self.framebuf.text(f'{string[13:]}', 0, int(core * self.height / 2 + self.height / 4), 1)
        self.show()
        self.lock.release()
...
class SSD1306_I2C(SSD1306):
    def __init__(self, width, height, i2c, lock, addr=0x3c, external_vcc=False):
        self.i2c = i2c
        self.addr = addr
